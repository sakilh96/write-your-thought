...
const UserFormModal = (props) => {
  ...
  const [initialValues, setInitialValues] = useState({
    ...
    type: props.usrdata ? props.usrdata.type : "Admin",
  ...
  const handleSubmit = async (values, actions) => {
    try {
      if (props.type == "Add") {
        const data = await userAddData(values);
        props.onHide();
        actions.setSubmitting(false);
        toast("User added successfully", "success");
      } else {
        const data = await useUpdateData(props.usrdata._id, values);
        props.onHide();
        actions.setSubmitting(false);
